...
class VITONDataset(data.Dataset):
    ...
    def get_parse_agnostic(self, parse, pose_data):
        ...
        if getattr(self.opt, 'lower_body', False):
    ...
    def get_img_agnostic(self, img, parse, pose_data):
        ...
        if not getattr(self.opt, 'lower_body', False):
            agnostic.paste(img, None, Image.fromarray(np.uint8(parse_lower * 255), 'L'))
        else:
            agnostic.paste((128, 128, 128), Image.fromarray(np.uint8(parse_lower * 255), 'L'))
    ...
    def __getitem__(self, index):
        ...
        result = {
            ...
            'cloth': c,
            'cloth_mask': cm,
            'raw_parse': np.array(parse),
            'raw_pose_data': pose_data
        }

        if getattr(self.opt, 'lower_body', False):
            cloth_lower_dir = osp.join(self.data_path, 'cloth-lower')
            cloth_lower_list = [f for f in sorted(os.listdir(cloth_lower_dir)) if not f.startswith('.')]
            if len(cloth_lower_list) > 0:
                l_name = cloth_lower_list[index % len(cloth_lower_list)]
                cl = Image.open(osp.join(cloth_lower_dir, l_name)).convert('RGB')
                cl = transforms.Resize(self.load_width, interpolation=2)(cl)
                cml = Image.open(osp.join(self.data_path, 'cloth-lower-mask', l_name))
                cml = transforms.Resize(self.load_width, interpolation=0)(cml)
                
                cl = self.transform(cl)
                cml_array = np.array(cml)
                cml_array = (cml_array >= 128).astype(np.float32)
                cml_tensor = torch.from_numpy(cml_array).unsqueeze_(0)
                
                # Assign using the first key (e.g. 'unpaired') to match cloth structure
                key = list(c_names.keys())[0] if 'c_names' in locals() else 'unpaired'
                result['cloth_lower'] = {key: cl}
                result['cloth_mask_lower'] = {key: cml_tensor}
        return result
